...
void vout_wmix_setup(struct tcc_vout_device *vout)
{
	...
	#if defined(CONFIG_TCC_VIOCMG)
	viocmg_set_wmix_ovp(VIOCMG_CALLERID_VOUT, vioc->wmix.id, 16/*vioc->wmix.ovp*/);
	#endif
...
void vout_onthefly_display_update(struct tcc_vout_device *vout, struct v4l2_buffer *buf)
{
	...
	if(buf->m.planes[MPLANE_VID].reserved[HEVC_COMP_MAP_EN]) {
		...
		dprintk("%s map converter: size: %dx%d  \n", displayed_bufs,  __func__, vout->src_pix.width, vout->src_pix.height);
	...
	#endif
	{
		...
		{
			printk(KERN_ERR "changing source (%dx%d)->(%dx%d)\n",
				vout->src_pix.width, vout->src_pix.height,
				buf->m.planes[MPLANE_VID].reserved[VID_WIDTH], buf->m.planes[MPLANE_VID].reserved[VID_HEIGHT]);
		...
		{
			printk("changing crop (%d,%d)(%dx%d)->(%d,%d)(%dx%d)\n",
...
static irqreturn_t vsync_irq_handler(int irq, void *client_data)
{
	...
	if(vioc->disp.addr->uLSTATUS.nREG & VIOC_DISP_IREQ_VSF_MASK) {
...
static void sub_rdma_alpha_setup(struct vioc_rdma *sub_rdma)
{
...
void vout_pop_all_buffer(struct tcc_vout_device *vout)
{
	...
	if(wait_event_interruptible_timeout(vout->frame_wait, vout->wakeup_int == 1, msecs_to_jiffies(100)) <= 0)
...
int vout_subplane_init(struct tcc_vout_device *vout)
{
	...
	struct vioc_rdma *sub_rdma_ext = &vioc->sub_rdma_ext;
	struct vioc_wmix *sub_wmix_ext = &vioc->sub_wmix_ext;
	int ret = 0;
	...
	sub_rdma_alpha_setup(sub_rdma);
	sub_rdma_setup(sub_rdma);
#if 0	// 20160329 IB Jeon
	/* 2. onthefly rdma */
	sub_rdma_ext->fmt = VIOC_IMG_FMT_ARGB8888;		// default: ARGB8888
	sub_rdma_ext->width = vout->src_pix.width;		// default: video size
	sub_rdma_ext->height = vout->src_pix.height;
	sub_rdma_ext->swap = 5;							// RGB3, or 5 (BGR3)
	sub_rdma_alpha_setup(sub_rdma_ext);
	sub_rdma_setup(sub_rdma_ext);
#endif
	/* 3. wmix */
	...
	sub_wmix_ext->left = 0;
	sub_wmix_ext->top = 0;
	sub_wmix_ext->width = vioc->wmix.width;
	sub_wmix_ext->height = vioc->wmix.height;
	VIOC_WMIX_GetOverlayPriority(sub_wmix_ext->addr, &sub_wmix_ext->ovp);
...
{
	struct tcc_vout_vioc *vioc = vout->vioc;
	struct vioc_rdma *onthefly_rdma = &vioc->sub_rdma_ext;
	struct vioc_wmix *onthefly_wmix = &vioc->sub_wmix_ext;
	struct vioc_rdma *m2m_rdma = &vioc->sub_rdma;
	struct vioc_wmix *m2m_wmix = &vioc->sub_wmix;
	...
	/* set 1st buf */
	if (0 == vioc->sub_buf_init) {
		vioc->sub_buf_init = 1;

		if(vout->onthefly_mode)
		{
			onthefly_rdma->width = alpha->width;
			onthefly_rdma->height = alpha->height;
			sub_rdma_setup(onthefly_rdma);
		}
		else
		{
			m2m_rdma->width = alpha->width;
			m2m_rdma->height = alpha->height;
			sub_rdma_setup(m2m_rdma);
		}
	}
	...
	/* check subtitle size */
	if(vout->onthefly_mode) {
		if ((onthefly_rdma->width != alpha->width) || (onthefly_rdma->height != alpha->height)) {
			onthefly_rdma->width = alpha->width;
			onthefly_rdma->height = alpha->height;
			sub_rdma_setup(onthefly_rdma);
		}
		/* change rdma base address */
		VIOC_RDMA_SetImageBase(onthefly_rdma->addr, onthefly_rdma->img.base0, onthefly_rdma->img.base1, onthefly_rdma->img.base2);
	} else {
		if ((m2m_rdma->width != alpha->width) || (m2m_rdma->height != alpha->height)) {
			m2m_rdma->width = alpha->width;
			m2m_rdma->height = alpha->height;
			sub_rdma_setup(m2m_rdma);
		}
		/* change rdma base address */
		VIOC_RDMA_SetImageBase(m2m_rdma->addr, m2m_rdma->img.base0, m2m_rdma->img.base1, m2m_rdma->img.base2);
	}

	/* check subtitle start position (x,y) */
	if(vout->onthefly_mode) {
		if ((onthefly_wmix->left != alpha->offset_x) || (onthefly_wmix->top != alpha->offset_y)) {
			onthefly_wmix->left = alpha->offset_x;
			onthefly_wmix->top = alpha->offset_y;
			printk("rdma (%dx%d) wmix ovp(%d) pos(%d) (%d,%d)(%dx%d)\n", onthefly_rdma->width, onthefly_rdma->height, onthefly_wmix->ovp, onthefly_wmix->pos,onthefly_wmix->left, onthefly_wmix->top, onthefly_wmix->width, onthefly_wmix->height);
			sub_wmix_setup(onthefly_wmix);
		}
	} else {
		if ((m2m_wmix->left != alpha->offset_x) || (m2m_wmix->top != alpha->offset_y)) {
			m2m_wmix->left = alpha->offset_x;
			m2m_wmix->top = alpha->offset_y;
			sub_wmix_setup(m2m_wmix);
		}
	}
